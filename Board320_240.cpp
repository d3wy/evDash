...
void Board320_240::menuItemClick()
{
  ...
  if (liveData->menuItemSelected > 0)
  {
    ...
    case MENU_SERIAL_CONSOLE:
      ...
      break;
    case MENU_DEBUG_LEVEL:
      liveData->settings.debugLevel = (liveData->settings.debugLevel == 3) ? 0 : liveData->settings.debugLevel + 1;
...
void Board320_240::mainLoop()
{
  ...
  if (liveData->params.currentTime - liveData->params.lastIgnitionOnTime > 10 && (liveData->params.currentTime - liveData->params.lastIgnitionOnTime < MONTH_SEC || liveData->params.lastIgnitionOnTime == 0) && liveData->settings.sleepModeLevel >= 1 && liveData->params.currentTime - liveData->params.lastButtonPushedTime > 10)
  {
    setBrightness(0);
  }
  else
  ...
  if (liveData->params.currentTime - liveData->params.lastIgnitionOnTime > 30 && (liveData->params.currentTime - liveData->params.lastIgnitionOnTime < MONTH_SEC || liveData->params.lastIgnitionOnTime == 0) && !liveData->params.chargingOn && liveData->settings.sleepModeLevel >= 2 && liveData->params.currentTime - liveData->params.wakeUpTime > 30 && liveData->params.currentTime - liveData->params.lastButtonPushedTime > 10)
  {
    if (liveData->params.sim800l_enabled)
    {
      sim800lSendData();
